...
def objective(loc_xy, rs):
    ...
    # err_sum = 0
    dists_pre = [
        [get_dist(loc_ap_xy[2 * ix_loc_ap:2 * ix_loc_ap + 2], loc_sp_xy[2 * ix_loc_sp:2 * ix_loc_sp + 2]) for ix_loc_ap
         in range(len(loc_ap_xy) // 2)] for ix_loc_sp in range(len(loc_sp_xy) // 2)]
...
class SelfCalibration:
    ...
    def gen_step_rangemeas(self, sigma=0):
        ...
        self.steps = [[np.random.rand() * np.random.rand() * np.cos(np.random.rand() * 2 * np.pi) - 5 + np.random.randn(),
                       perim * np.random.rand() * np.sin(np.random.rand() * 2 * np.pi)]
                      for _ in np.arange(self.M)]
        self.steps = np.asarray(self.steps)
    ...
    def update_step_converge(self, sigma=0, mds_init=False, n_round_thrd=20):
        ...
        if mds_init:
            loc_ap_xy_init = self.mds_init(sigma=sigma)
            initi_guess[:2*(self.N-1)] = loc_ap_xy_init[2:]
    ...
    def mds_init(self, sigma=0):
        # from pylocus.mds import MDS
        ...
        def gen_dist_mat_symm(dists):
            ...
            dist_mat = np.asarray(dist_mat)
            n_i, n_j = np.shape(dist_mat)

            for i in range(n_i-1):
                for j in range(i+1, n_i):
                    e_i = dist_mat[i, i]
                    e_j = dist_mat[j, j]
                    # take the measure from a step closer to the anchor
                    if e_i < e_j:
        ...
        # dist_mat = gen_dist_mat_naive(dists)
        dist_mat = gen_dist_mat_symm(dists)
        # res = MDS(dist_mat, dim=2, method='geometric')
        emb = MDS(n_components=2, dissimilarity='precomputed')
        X_transformed = emb.fit_transform(dist_mat)
        shape = X_transformed.shape
        X_transformed = X_transformed - X_transformed[0]
        X_transformed_ = X_transformed.flatten()

        # self.vis_rot_loc(X_transformed_, self.loc_xy)
    ...
    def update_step_nrounds_mds(self, sigma=0, n_round=1):
        ...
        self.initi_guess = np.random.randn(2 * (self.M + self.N - 1))
        self.initi_guess[:2*(self.N-1)] = loc_ap_xy_init[2:]
    ...
    def iterations(self, N_iter=500, sigma=0.1, n_round=1):
        ...
        return np.asarray(diversity_var_s), np.asarray(res_err_range_s) / (self.M + self.N), np.asarray(
            res_err_rot_s) / (self.M + self.N), np.asarray(loc_ap_xy_s), np.asarray(loc_tg_xy_s), np.asarray(dists_list)

    def iterations_converge(self, N_iter=800, sigma=0.1, mds_init=True, n_round_thrd=20):
        from tqdm import tqdm
        diversity_var_s, res_err_range_s, res_err_rot_s = [], [], []
        loc_ap_xy_s, loc_tg_xy_s = [], []
        dists_list = []
        ...
        for ix in tqdm(np.arange(N_iter)):
            ...
            diversity_var, res_err_range, res_err_rot, cnt_rounds, flag = self.update_step_converge(sigma=sigma, mds_init=mds_init, n_round_thrd=n_round_thrd)
    ...
    def get_rot_opt(self, loc_ap_xy, loc_tg_xy, method='SLSQP'):
        ...
        # ALIGN WITH ROTATION after 1st round
        anchor_pairs = [loc_ap_xy[2:self.N*2], loc_tg_xy[2:self.N*2]]
        ...
        # rotation
        loc_x_rot, loc_y_rot = rotate_xy(loc_ap_xy[2:self.N*2], theta=rot)
        loc_x_rot, loc_y_rot = np.concatenate([[0], loc_x_rot]), np.concatenate([[0], loc_y_rot])

        loc_ap_xy[np.arange(2, self.N*2, 2)] = loc_x_rot[1:]
        loc_ap_xy[np.arange(2, self.N*2, 2) + 1] = loc_y_rot[1:]
...
def vis_converge(m_step=15, n_anch=5, sigma=0.3, postfix='_', n_round_thrd=20):
    f_loc_ap_xy_s = 'loc_ap_xy_s.npy'
    f_loc_tg_xy_s = 'loc_tg_xy_s.npy'
    f_dists = 'dists.npy'
    directory = os.getcwd()
    pth_res = os.path.join(directory, 'res_loc_convg_'+postfix)
    ...
    #     ix_cnt_4 = np.concatenate([(n_anch-1) * ix_cnt_ + i for i in range(n_anch-1)])
    #     diff_a_xys_dict[str(cnt)] = diff_a_xys_all[ix_cnt_4]
    #
    #     # indices of step nodes
    #     ix_cnt_5 = np.concatenate([m_step * ix_cnt_ + i for i in range(m_step)])
    ...
    # draw_boxplot_s(pth_res, time_dict.values(), labels=time_dict.keys(), ylabel='Time Elapsed (s)',
    #                title='time elapsed')
    ...
    return  diff_a_xys_all, diff_s_xys_all
...
def simu_converge(m_step=5, n_anch=5, sigma=0.3, postfix='_', mds_init=True, n_round_thrd=20):
    ...
    calib = SelfCalibration(M=m_step, N=n_anch)
    ...
    diversity_var, res_err_range, res_err_rot, loc_ap_xy_s, loc_tg_xy_s, dists, cnt_list, time_list, flag_list = \
        calib.iterations_converge(sigma=sigma, mds_init=mds_init, n_round_thrd=n_round_thrd)
...
def eval_simu(n_round=1, m_steps=range(3, 8), sigmas=np.arange(0.0, 0.5, 0.1)):
    ...
    for mm in m_steps:
        ...
        for sigma in sigmas:
            sigma = np.round(sigma, decimals=3)
            diversity_var, res_err_range, res_err_rot, loc_ap_xy_s, loc_tg_xy_s, dists = calib.iterations(sigma=sigma,
                                                                                                          n_round=n_round)
...
def eval_convergence(m_step=15, n_anch=5, sigma=0.3, postfix='_', mds_init=True, n_round_thrd=20):
    # simu_converge(m_step=m_step, n_anch=n_anch, sigma=sigma, postfix=postfix, mds_init=mds_init, n_round_thrd=n_round_thrd)

    diff_a_xys_all, diff_s_xys_all  = vis_converge(m_step=m_step, n_anch=n_anch, sigma=sigma, postfix=postfix, n_round_thrd=n_round_thrd)
    return diff_a_xys_all, diff_s_xys_all



def test_mds_exp():
    calib = SelfCalibration(M=50)
    # plt.show()
    res_err_range_dict = {}
    res_err_rot_dict = {}
    # calib.mds_test()
...
if __name__ == '__main__':
    ...
    #     eval_convergence(m_step=15, n_anch=n, mds_init=False, postfix='mds_{}_15_100_s'.format(n), n_round_thrd=100)
    # test_mds_exp()
    diff_a_xs_all_s, diff_a_ys_all_s, diff_a_xys_all_s = [], [], []
    diff_s_xs_all_s, diff_s_ys_all_s, diff_s_xys_all_s = [], [], []
    labels = []
    ...
    for n in range(3, 10, 2):
        diff_a_xys_all, diff_s_xys_all = eval_convergence(m_step=15, n_anch=n, mds_init=False, postfix='mds_{}_15_20_s'.format(n), n_round_thrd=100)

        # if n != 3:
        #     diff_a_xys_all = np.sort(diff_a_xys_all)
        #     diff_s_xys_all = np.sort(diff_s_xys_all)
        #     len_n = len(diff_a_xys_all)
        #     len_f = int(1 * len_n)
        #     diff_a_xys_all = diff_a_xys_all[:len_f]
        #     diff_s_xys_all = diff_s_xys_all[:len_f]
        # else:
        #     diff_a_xys_all = np.array(np.sort(diff_a_xys_all))
        #     diff_s_xys_all = np.array(np.sort(diff_s_xys_all))
        #     len_n = len(diff_a_xys_all)
        #     len_f = int(0.999 * len_n)
        #     diff_a_xys_all = np.array(diff_a_xys_all[:len_f])
        #     diff_s_xys_all = np.array(diff_s_xys_all[:len_f])

        diff_a_xys_all = np.array(np.sort(diff_a_xys_all))
        diff_a_t = list(diff_a_xys_all[-100:])
        diff_s_xys_all = np.array(np.sort(diff_s_xys_all))
        diff_s_t = list(diff_s_xys_all[-100:])

        len_n = len(diff_a_xys_all)
        # ratio = 0.96
        # len_f = int(0.91 * len_n)
        # len_n_ = len(diff_s_xys_all)
        # len_f_ = int(ratio * len_n_)
        # diff_a_xys_all = np.array(diff_a_xys_all[:len_f])
        ...
        diff_a_xys_all_s.append(diff_a_xys_all)
        diff_s_xys_all_s.append(diff_s_xys_all)
        labels.append(r'{}={}'.format('#sensor', n))

    directory = os.getcwd()
    pth_res_ = os.path.join(directory, 'res_loc')
    # pth_res = os.path.join(pth_res_, 'step_{}_sigm_{}'.format(15, 20))
    postfix = 'res_n_{}_sigma_{}_{}'.format(15, int(15 * 10), 20)
    draw_cdf_e_s(pth_res_, diff_a_xys_all_s, labels=labels, title='Loc_anchor_xy_{}_{}'.format(param_i, mm))

    draw_cdf_e_s(pth_res_, diff_s_xys_all_s, labels=labels, title='Loc_step_xy_{}_{}'.format(param_i, mm))