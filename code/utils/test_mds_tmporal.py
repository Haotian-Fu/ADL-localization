...
def draw_cdf_e_s(directory, devis, labels, title='Dist Diff'):
    ...
    if not os.path.exists(os.path.join(directory, 'fig')):
        os.mkdir(os.path.join(directory, 'fig'))
    fig.savefig(os.path.join(directory, 'fig', title + '_cdf.png'), bbox_inches='tight')
...
class SelfCalibration:
    ...
    def update_step_converge(self, sigma=0, mds_init=False, n_round_thrd=20):
        ...
        self.loc_ap_xy, self.loc_sp_xy, res_err_range = self.get_loc_opt(self.dists, self.initi_guess)
        # self.vis_loc_est(self.loc_ap_xy, self.loc_sp_xy)
        self.vis_rot_loc(self.loc_ap_xy, np.concatenate([np.zeros(2),initi_guess[:2 * self.N - 2]]), title='loc mds 0')
        self.vis_rot_loc(self.loc_ap_xy, self.loc_xy, title='loc mds')
        ...
        while np.sqrt(np.sum((initi_guess[:2 * self.N - 2] - self.loc_ap_xy[2:]) ** 2) / (self.N - 1)) > threshold:
            ...
            if cnt_rounds > n_round_thrd:
                ...
                break
            # self.vis_loc_est(self.loc_ap_xy, self.loc_sp_xy)
            self.vis_rot_loc(self.loc_ap_xy, np.concatenate([np.zeros(2), initi_guess[:2 * self.N - 2]]), title='loc mds_{}'.format(cnt_rounds))
            self.vis_rot_loc(self.loc_ap_xy, self.loc_xy, title='loc mds')
    ...
    def mds_init(self, sigma=0):
        ...
        # dist_mat = np.asarray([[0,4,5,3], [4,0,3,5],[5,3,0,4], [3,5,4,0]])
        ...
        # X_transformed_ = np.concatenate([X_transformed_, np.zeros(2)])
        # X_2 = X_transformed_ ** 2
        self.vis_rot_loc(X_transformed_, self.loc_xy, title='loc mds')
    ...
    def update_step_nrounds_mds(self, sigma=0, n_round=1):
        ...
        # self.vis_loc_est(self.loc_ap_xy, self.loc_sp_xy)
        #
        self.vis_rot_loc(self.loc_ap_xy, self.loc_xy, title='loc mds 0')
        ...
        for i in range(1, n_round):
            ...
            self.loc_ap_xy, self.loc_sp_xy, res_err_range = self.get_loc_opt(self.dists, initi_guess)
            #
            self.vis_rot_loc(self.loc_ap_xy, initi_guess[:2 * self.N - 2], title='loc mds_{}'.format(i))
    ...
    def iterations_converge(self, N_iter=2000, sigma=0.1, mds_init=True, n_round_thrd=20):
        from tqdm import tqdm
        diversity_var_s, res_err_range_s, res_err_rot_s = [], [], []
        loc_ap_xy_s, loc_tg_xy_s = [], []
        dists_list = []
    ...
    def vis_rot_loc(self, loc_ap_xy, loc_tg_xy, title='loc rot'):
        # VISUALIZE THE 1ST ESTIMATE OF LOCATIONS
        plt.figure()
        x = loc_ap_xy[np.arange(self.N) * 2]
        y = loc_ap_xy[np.arange(self.N) * 2 + 1]
        plt.scatter(x, y, marker='^', label='anchor')
        x_ = loc_tg_xy[np.arange(self.N) * 2]
        y_ = loc_tg_xy[np.arange(self.N) * 2 + 1]
        plt.scatter(x_, y_, marker='*', label='target')

        n_len = len(x)
        for i in range(n_len):
            x_seg = [x[i], x_[i]]
            y_seg = [y[i], y_[i]]
            plt.plot(x_seg, y_seg)
...
def vis_converge(m_step=5, sigma=0.3, postfix='_'):
...
def simu_converge(m_step=5, sigma=0.3, postfix='_', mds_init=True, n_round_thrd=20):
...
def eval_convergence(m_step=15, sigma=0.3, postfix='_'):
    simu_converge(m_step=m_step, sigma=sigma, postfix=postfix, mds_init=True, n_round_thrd=20)
    vis_converge(m_step=m_step, sigma=sigma, postfix=postfix)
...
if __name__ == '__main__':
    ...
    eval_convergence(m_step=5, postfix='mds_5_10_')
    # test_mds_exp()